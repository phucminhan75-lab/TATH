...
function speak(text, rate){
  if(!window.speechSynthesis || !S.sound) return;
  try{
...
function rMCQ(q){
  ...
  const visual = q.bigLetter ? `<div class="letterbig">${esc(q.bigLetter)}</div>`
    : q.auto  ? `<button class="speaker pulse" id="spk">🔊</button>`
...
function rInput(q){
  ...
  const visual = q.auto ? `<button class="speaker pulse" id="spk">🔊</button>`
               : q.big  ? `<span class="bigem">${q.big}</span>` : "";
  $("#qArea").innerHTML=`
   ${passageHTML(q)}
   <div class="qcard">
  ...
  try{
    const r = await fetch(HNA_SB.url,{ method:"POST",
      ...
      body: JSON.stringify({ ho_ten_be:f.name, sdt_me:f.phone, email:f.email,
        lop:+f.grade, khu_vuc:f.area, thiet_bi:(navigator.userAgent||"").slice(0,200), nguon:"app" })
    });
    return r.ok;
...
function logout(){
  ...
  S.user=null; S.userSent=""; save(); loginLabel(); enforceGate();
}
function toast(msg){
  let el=$("#toast");
